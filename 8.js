...
async function main() {
    ...
    let isNeboOrPodzemActive = false;
    let isFailAfter6 = false;
    let isFailAfter7 = false;
    const neboCoords = {x: 1638, y: 301};
    const podzemCoords = {x: 1671, y: 302};
    ...
    while (success <= 8) {
        ...
        if (success === 6 && !isFailAfter7) {
            backgroundLeftClick(targetWindow, 1717, 700);
            await pauseAwareSleep(2000);
            backgroundRightClick(targetWindow, podzemCoords.x, podzemCoords.y);
            await pauseAwareSleep(2000);
        }
